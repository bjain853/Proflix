...
const UserActions = {
		...
		return new Promise((resolve, reject) => {
			getConnection(function (connection) {
				...
				query(`SELECT * FROM ${users} WHERE username = ?`, [username])
					.then(([rows]) => {
						if (rows.length === 0) {
							return reject(new Error('Password/Username Incorrect'));
						}
						const user = rows[0];
						return resolve({
	...
	deleteUser: (uId) => {
		...
		return new Promise((resolve, reject) => {
			getConnection(function (connection) {
				connection.
					execute(`DELETE FROM ${users} WHERE uId = ?`, [uId]).catch((err) => {
						if (err) {
							return reject(err);
						} else {
	...
	addUser: (name, username, email, password) => {
		return new Promise((resolve, reject) => {
			...
			bcrypt.hash(password, 10, (err, hash) => {
				...
				getConnection(function (connection) {
					connection
						.execute(`INSERT INTO ${users} SELECT ?,?,?,?,?`, [uId, username, name, email, hash])
